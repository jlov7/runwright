...
- Status: Done (mode architecture and mode-primary CTA shipped in `apps/web/index.html`, `apps/web/navigation.js`, and `apps/web/app.js` with updated navigation regression coverage in `tests/frontend-navigation-disclosure.test.ts`)
...
- Status: Done (backdrop + dialog semantics + focus trap/return + escape behavior implemented in `apps/web/app.js` and `apps/web/index.html`; coverage updated in `tests/frontend-accessibility-responsive.test.ts` and `tests/game-runtime-shell.test.ts`)
...
- Status: Done (focus token fixed, handle input error semantics added, and safe-area rules shipped in `apps/web/styles.css` + `apps/web/index.html` + `apps/web/app.js`; verified by `tests/frontend-accessibility-responsive.test.ts`)
...
- Status: Done (implemented via `apps/web/navigation.js`, progressive disclosure shell updates in `apps/web/index.html`, and regression coverage in `tests/frontend-navigation-disclosure.test.ts`)
...
- Status: Done (guided journey strip, staged CTA sequencing, explicit rationale copy, and actionable error/empty-state recovery messaging shipped in `apps/web/index.html` with updated visual/runtime tests)
...
- Evidence: Recent pushed heads fail before workflow steps execute because GitHub Actions cannot start jobs due to account billing limits. Sample `CI`/`CodeQL` run pairs: `22109118266`/`22109118280`, `22110929744`/`22110929736`, `22111146887`/`22111146880`, `22112492942`/`22112492922`, `22112524863`/`22112524890`, `22203952764`/`22203952755`, `22240155561`/`22240155541`, `22240196616`/`22240196630`, `22243078342`/`22243078364`, `22243679198`/`22243679170`, `22243878530`/`22243878522`, `22244054296`/`22244054299`, `22244385629`/`22244385634`, `22244568988`/`22244568976`, `22245086541`/`22245086546`, `22245314513`/`22245314506`, `22261941895`/`22261941891`, `22262071635`/`22262071634`, and latest evaluated head `9820260` reruns `22262138782`/`22262138797` (attempt `2`) with all failing jobs still reporting `steps: []` / `stepsCount: 0`; all annotate startup failure and billing/spending-limit guidance.
