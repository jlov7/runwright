...
describe("workflow policy", () => {
  it("ci workflow uses least-privilege defaults and explicit job timeouts", () => {
    ...
    expect(sarifJob?.needs).toBe("verify");
    ...
    expect(mutationJob?.needs).toBe("verify");
    ...
    expect(fuzzJob?.needs).toBe("verify");
    ...
    expect(sbomJob?.needs).toBe("verify");
    ...
    expect(compatibilityJob?.needs).toBe("verify");
    ...
    expect(performanceTrendJob?.needs).toBe("verify");
    ...
    expect(scorecardJob?.if).toBe("always()");
    ...
    expect(scorecardNeeds).toContain("verify");
    ...
    expect(hasRunContaining(scorecardSteps, "--check verify=${{ needs.verify.result }}")).toBe(true);
