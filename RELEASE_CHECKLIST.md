...
- [x] Evaluated release head `8f7db16` has fresh compensating local evidence: `pnpm verify`, `pnpm run doctor` (`reports/doctor/doctor.json` generated `2026-02-20T23:26:57.242Z`), full `pnpm ship:gate` (`reports/quality/ship-gate.summary.json` generated `2026-02-20T23:29:53.651Z`, `overall.ok: true`), `pnpm quality:evidence:verify` (`ok: true`), and `pnpm release:verify-local` (`release artifact verification: ok`).
- [x] CI/CodeQL rerun on evaluated release head (`8f7db16`) confirms unchanged startup-failure mode after manual reruns: `CI` `22245086541` and `CodeQL` `22245086546` both failed with zero-step jobs (`steps: []`), so RG-007 exception remains active with fresh compensating local evidence.
- [x] Prior head `82c7d6a` has fresh compensating local evidence: `pnpm verify`, `pnpm run doctor` (`reports/doctor/doctor.json` generated `2026-02-20T23:04:08.871Z`), full `pnpm ship:gate` (`reports/quality/ship-gate.summary.json` generated `2026-02-20T23:06:58.563Z`, `overall.ok: true`), `pnpm quality:evidence:verify` (`ok: true`), and `pnpm release:verify-local` (`release artifact verification: ok`).
- [x] CI/CodeQL rerun on prior head (`82c7d6a`) confirms unchanged startup-failure mode: `CI` `22244568988` and `CodeQL` `22244568976` both failed with zero-step jobs (`steps: []`), so RG-007 exception remains active with fresh compensating local evidence.
