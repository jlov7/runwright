...
- [x] Latest full local gates on `2026-02-21` are green on pushed head `aac19d2`: `pnpm verify` (390/390 tests passing, build green), `pnpm run doctor` (`reports/doctor/doctor.json` generated `2026-02-21T18:25:52.005Z`, `overall.ok: true`), full `pnpm ship:gate` (`reports/quality/ship-gate.summary.json` generated `2026-02-21T18:30:06.112Z`, `overall.ok: true`), `pnpm quality:evidence:verify` (`reports/quality/evidence-verification.json`, `ok: true`), and `pnpm release:verify-local` (`release artifact verification: ok`).
- [x] CI/CodeQL reruns on latest pushed head `aac19d2` confirm unchanged startup-failure mode after explicit rerun attempts: `CI` run `22261941895` attempt `2` and `CodeQL` run `22261941891` attempt `2` both failed with zero-step jobs (`steps: []`), so RG-007 exception remains active with fresh compensating local evidence.
